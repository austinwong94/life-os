...
The release gate contains **201 checks plus 29 WebKit repeats**, including two new core and five browser scenarios. It covers original Planner card entry, Calendar/Tasks edits, completion on selected past dates, archive/restore, long future dates, board/area isolation, ongoing activity grouping, two-tab changes, retained unfinished activity notes and phone/portrait layouts. The first candidate failed on aborted development file-watcher requests reported by WebKit during reload. Responsive/planning test pages explicitly use `liveReload=0`, with a separate regression proving normal previews still watch files. Error assertions remain strict. No production records are test fixtures. No SQL migration or bulk data replacement is needed. Follow [release checks](https://github.com/austinwong94/life-os/actions) for the exact candidate and deployment outcome.
...
The full release workflow passed **189 distinct automated checks**, with **19 responsive/planning scenarios repeated in WebKit**, on `ecf72b9`. A separate candidate run exposed an intermittent editor autofocus race: a delayed callback could move focus from Notes to Task name. This follow-up guards that callback and adds a deterministic regression, bringing the suite to **190 checks plus 20 WebKit repeats**. The failed candidate was investigated rather than dismissed because another run passed. See [verified baseline](https://github.com/austinwong94/life-os/actions/runs/34478213762) and [current release checks](https://github.com/austinwong94/life-os/actions) for final release status rather than inferring deployment from this source file.
